...
const KEYWORDS = new Set([
  "bool", "char", "char8_t", "char16_t", "char32_t", "int", "long", "short", "signed",
  "unsigned", "float", "double", "void", "wchar_t", "auto", "const", "static", "extern",
  "register", "mutable", "volatile", "thread_local", "if", "else", "switch", "case",
  "default", "for", "while", "do", "break", "continue", "goto", "return", "true", "false",
  "nullptr", "class", "struct", "union", "enum", "friend", "this", "virtual", "explicit",
  "inline", "final", "override", "public", "private", "protected", "template", "typename",
  "using", "concept", "requires", "try", "catch", "throw", "noexcept", "const_cast",
  "dynamic_cast", "reinterpret_cast", "static_cast", "decltype", "typeid", "namespace",
  "export", "import", "module", "and", "and_eq", "or", "or_eq", "not", "not_eq", "bitand",
  "bitor", "compl", "xor", "xor_eq",
]);
  ...
  while (i < code.length) {
    ...
    if (c === '"') {
      ...
      while (i < code.length && code[i] !== '"') {
        buf += code[i];
        steps.push({ index: i, log: `> Reading message char '${code[i]}'`, cppLine: 14 });
        i++;
      }
      if (i < code.length) {
        buf += '"';
    ...
    if (isDigit(c) || isNegNum) {
      ...
      if (isNegNum) {
        buf += "-";
        steps.push({ index: i, log: `> Negative sign for number`, cppLine: 21 });
        i++;
      }
      while (i < code.length && isDigit(code[i])) {
        buf += code[i];
        steps.push({ index: i, log: `> Reading digit '${code[i]}'`, cppLine: 22 });
        i++;
      }
      steps.push({
        index: i - 1,
    ...
    if (isAlpha(c)) {
      ...
      while (i < code.length && isAlnum(code[i])) {
        buf += code[i];
        steps.push({ index: i, log: `> Reading word char '${code[i]}'`, cppLine: 28 });
        i++;
      }
      if (isKeyword(buf)) {
        steps.push({
...
void scanCode(const string& code) {
    ...
    while (i < code.size()) {
        ...
        if (c == '"') {
            ...
            else                 cout << "IDENTIFIER: " << word << endl;
        }
        else if (isSymbol(c)) {
            cout << "SYMBOL: " << c << endl;
            i++;
